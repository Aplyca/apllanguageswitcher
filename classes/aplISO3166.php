...
class AplISO3166
{
    ...
    function AplISO3166( $address = null )
    {
        if ( ! $address )
            $this->address = AplISO3166::getRealIpAddr();
        else
    ...
    {
        if ( array_key_exists( "HTTP_CLIENT_IP", $_SERVER ) and AplISO3166::validip( $_SERVER["HTTP_CLIENT_IP"] ) )
        {
        ...
        {
            if ( AplISO3166::validip( trim( $ip ) ) )
            {
                return $ip;
            }
        }
        }
        if ( array_key_exists( "HTTP_X_FORWARDED", $_SERVER ) and AplISO3166::validip( $_SERVER["HTTP_X_FORWARDED"] ) )
        {
            return $_SERVER["HTTP_X_FORWARDED"];
        }
        elseif ( array_key_exists( "HTTP_FORWARDED_FOR", $_SERVER ) and AplISO3166::validip( $_SERVER["HTTP_FORWARDED_FOR"] ) )
        {
            return $_SERVER["HTTP_FORWARDED_FOR"];
        }
        elseif ( array_key_exists( "HTTP_FORWARDED", $_SERVER ) and AplISO3166::validip( $_SERVER["HTTP_FORWARDED"] ) )
        {
            return $_SERVER["HTTP_FORWARDED"];
        }
        elseif ( array_key_exists( "HTTP_X_FORWARDED", $_SERVER ) and AplISO3166::validip( $_SERVER["HTTP_X_FORWARDED"] ) )
        {
            return $_SERVER["HTTP_X_FORWARDED"];
        }
        else
    ...
    {
        $regionini = eZINI::instance( 'geoip.ini' );
        return $regionini->variable( 'Settings', 'DefaultCountryCode' );
    }
    ...
    {
        $ip = new AplISO3166( $address );
        $code = $ip->getCCfromIP();
		
        if ( !$code )
		{
            $code = AplISO3166::defaultCountryCode();
        }
